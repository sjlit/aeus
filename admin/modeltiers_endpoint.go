...
//
// Column policy: parent / label / value must all appear in the
// selectable allowlist derived from sys_schemas (read scenarios,
// non-password format) — see endpoint_allowlist.go.  Unknown or
// write-only columns are rejected with 1001 Invalid.
//
...
// models that actually declare a tenant_id column — global models are
// queried unscoped because rest/v3 appends `tenant_id = ?`
// unconditionally whenever a non-empty tenant is supplied.
//
// Auth:    must be mounted behind the JWT middleware, and the route
//
//	template "GET /rest/model-tiers/:module/:table" carries its own
...
func RegisterModelTiersEndpoint(s *Server) (http.HandlerFunc, error) {
	...
	h := func(w http.ResponseWriter, r *http.Request) {
		...
		if err := requireSelectableColumns(r.Context(), s.opts.DB, module, table, parent, label, value); err != nil {
			writeEnvelope(w, int(errs.CodeInvalid), err.Error(), nil)
			return
		}
		var tenant string
